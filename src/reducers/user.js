...
export function loadPreferences() {
  return function(dispatch) {
    ...
    getCache('prefs-' + userAddress).then(val => {
...
export function savePreferences(preferences) {
  return function(dispatch) {
    const userAddress = store.getState().user.userAccount;
    setCache('prefs-' + userAddress, JSON.stringify(preferences)).then(val => {
      return dispatch({
        type: 'LOCAL_PREFS_LOAD',
        payload: preferences
      });
...
export function clearPreferences() {
  return function(dispatch) {
    const userAddress = store.getState().user.userAccount;
    clearCache('prefs-' + userAddress).then(val => {
      return dispatch({ type: 'LOCAL_PREFS_CLEAR' });
    });
  };
}
