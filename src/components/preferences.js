...
class DetailComponent extends Component {
  ...
  syncToCloud() {
    // save to server
    this.props.savePreferences_server({ text: this.props.text });
  }
  ...
  render() {
    return (
      <div>
        <h1>
          {' '}
          <Link to="/">Demos</Link>&nbsp;> Preferences{' '}
        </h1>
        <hr />
        <p>
          We'll automatically save the data on the user's device using the
          public key as an identifier.
        </p>
        <p>
          For saving to the cloud we'll require that the user digitally sign the
          message so that we can use the signature for authentication on the
          server.
        </p>
        ...
        <div style={buttonGrid}>
          <button
            ...
            className={this.setActive('text', 'smaller')}
          >
            Smaller
          </button>
          <button
            onClick={this.setPreference.bind(this, 'text', 'medium')}
            className={this.setActive('text', 'medium')}
          >
            Default
          </button>
          <button
            onClick={this.setPreference.bind(this, 'text', 'larger')}
            className={this.setActive('text', 'larger')}
          >
            Larger
          </button>
        </div>
...
const mapDispatchToProps = dispatch => {
  return {
    ...
    savePreferences_server: preferences => {
      dispatch(savePreferences_server(preferences));
    },
    clearPreferences_server: () => {
      dispatch(clearPreferences_server());
    }
  };
};
